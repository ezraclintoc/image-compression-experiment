...
runner/
  src/main.rs               — CLI: commands, CSV export, image collection
  src/compressors.rs        — registry (the single place compressors are listed)
  src/bin/gen.rs            — generator for the math/ test images
images/                     — source images, grouped by content (see images/README.md)
  {math,cartoon,real-life,screenshots,gradients,textures}/
```

Images are loaded directly (PNG, JPG, BMP, WebP, PPM, …) — there is no separate
conversion step.

## Running

Pass image paths and compressor names as plain arguments, in any order. Paths
may be files or directories (walked recursively); anything that isn't an
existing path is read as a compressor name.

```bash
# Stats for one image (all compressors by default)
cargo run -p runner --release -- images/real-life/kodim05.png

# One image, one compressor
cargo run -p runner --release -- images/real-life/kodim05.png Huffman

# Average stats across a directory
cargo run -p runner --release -- images/math RLE

# Two compressors on a directory, side by side
cargo run -p runner --release -- images/math RLE Huffman

# No path given: run over everything in images/
cargo run -p runner --release -- Huffman

# Write per-image/per-compressor rows to CSV
cargo run -p runner --release -- images/ --csv results.csv

# Regenerate the 25 math test images
cargo run -p runner --bin gen
```

Compressor names (`PNG`, `RLE`, `RLCE`, `Huffman`, or `all`) are matched
case-insensitively and may also be given comma-separated or via
`-c, --compressor`.

The output format is inferred: a single image prints full stats, a single
compressor prints one averaged line per image category, and several compressors
print category × compressor tables for ratio, compress time, and decompress
time. Add `image`, `average`, or `compare` as an argument to force one.

Aggregate output is always grouped by image category (the directory under
`images/`) — there is no whole-run average and no per-image lines. For raw
per-image numbers use `--csv`.
...
The 25 generated images in `images/math/` are designed to expose strengths and weaknesses of specific algorithms:
...
| `images/math/` | Coded with AI assistance |
...
5. Register it in `runner/src/compressors.rs` — the single source of truth.
   Add a `use` line and one entry to `all_compressors()`:
   ```rust
   ("MyAlgo", Box::new(MyCompressor::new())),
   ```
   The CLI `--compressor` flag, CSV export, and the TUI picker all read from
   this list, so there's nothing else to wire up.
